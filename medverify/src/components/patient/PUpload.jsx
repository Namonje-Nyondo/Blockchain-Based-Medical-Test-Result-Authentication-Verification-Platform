...
export default function PUpload({ onComplete, onCancel }) {
  ...
  const [hashing, setHashing]     = useState(false);   // true while SHA-256 is computing
  const [hashResult, setHashResult] = useState(null);  // full HashResult object
  ...
  /* ── Step 1 & 2: Pick file → compute real SHA-256 ── */
  const pickFile = async (f) => {
    ...
    setHashing(true);
    try {
      /* Only file hash at this stage — metadata isn't filled yet */
  ...
  /* ── Step 4 + 5: Final submit — hash metadata + file together ── */
  const handleSubmit = async () => {
    setSubmit(true);
    try {
      const selectedType = RECORD_TYPES.find(t => t.id === recType);
      /* Re-hash with full metadata now that we have all fields */
      const finalResult = await hashMedicalFile(file, {
        recordType:  selectedType?.label || "Other",
        title,
        issuedBy:    doctor,
        institution,
        issuedAt:    date,
